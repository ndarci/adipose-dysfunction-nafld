...
nafld = names(ld)[!(names(ld) %in% iv$hit2)]

# clip ld matrix to just ivs and nafld variants
iv_nafld = ld[iv$hit2, nafld]
...
iv.ld = which(iv_iv > ld_thresh, arr.ind = T) %>% 
					...
					filter(row != col) %>%
					arrange(row) %>%
                    mutate(hit2 = rownames(iv_iv)[row])
...
iv.stat.ld = iv.stat %>% group_by(hit2) %>%
                        summarise(gene_ID = gene_ID,
                                fdr_eqtl = fdr_eqtl,
                                p_gwas = p_gwas) %>%
                        filter(hit2 %in% iv.ld$hit2)

# # find snp with minimum p-values in each one
# sig.cis = iv.stat.ld[which.min(iv.stat.ld$fdr_eqtl),]$hit2
# sig.gwas = iv.stat.ld[which.min(iv.stat.ld$p_gwas),]$hit2

# if(sig.cis != sig.gwas) {
#     print('single SNP does not have most signif cis FDR and GWAS pvalue')
# } else {
#     keepIV = sig.cis
    keepme = c('rs468221', 'rs3214023')

    # select the set of IVs that pass LD checks and go to the MR analysis
    iv.out = iv %>% filter(!(hit2 %in% iv.ld$hit2) | hit2 %in% keepme)
    write.table(iv.out, '../../data/iv_snps_cond_colocalized_ldpruned.txt')

    write.table(iv.out$hit2, '../../data/iv_snps_cond_colocalized_ldpruned_just_rsID.txt', row.names = F, col.names = F, quote = F)
# }
