...
iv.exp = iv %>% left_join(gwas.tg, by = c('hit2' = 'SNP')) %>%
			select(hit2, BETA, SE, ALLELE1, ALLELE0, A1FREQ,
...
iv.outcome = iv %>% left_join(gwas.nafld, by = c('hit2' = 'SNP')) %>%
			select(hit2, BETA, SE, ALLELE1, ALLELE0, A1FREQ,
					CHR, BP, P_BOLT_LMM_INF) %>%
			rename(SNP = 'hit2',
					beta = 'BETA',
...
cis.info = cis.info %>% filter(hit2 %in% df.harm$SNP) %>%
                    select(hit2, gene_symbol, beta_eqtl, fdr_eqtl,
                            beta_gwas, p_gwas) %>%
                    unique()
