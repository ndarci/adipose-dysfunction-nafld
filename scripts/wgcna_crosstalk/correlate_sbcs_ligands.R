...
# also correlate with the liver WGCNA modules, not just all the genes

# import the liver network
library(WGCNA)
...
l.geneTree = geneTree
# recalculate the MEs with colors
l.MEs = orderMEs(moduleEigengenes(l.datExpr, l.moduleColors)$eigengenes)
colnames(l.MEs) = paste0("L_", colnames(l.MEs))
# only keep the liver modules that correlate with steatosis, fibrosis, and NASH
keeplivermodules = paste0('L_ME', c('saddlebrown', 'cyan', 'tan', 'magenta', 
	'lightcyan1', 'lightgreen', 'darkgrey', 'violet', 'darkmagenta', 'royalblue'))
# merge the liver module MEs onto the liver expression data
l.cpm = l.cpm %>% 
		mutate(id = rownames(l.cpm)) %>% 
		left_join(l.MEs %>% 
				select(keeplivermodules) %>% 
				mutate(id = rownames(l.MEs)), 
				by = 'id') %>%
		select(-id)
...
# select the module correlations
livmodulecorr = allcorr %>% filter(grepl('L_', othergene_liver))
write.table(livmodulecorr, paste0('../../data/sbc_all_liver_module_correlations', rep('_healthy', healthy), '.txt'))

# select just the gene correlations
livgenecorr = allcorr %>% filter(!grepl('L_', othergene_liver))
write.table(livgenecorr, paste0('../../data/sbc_all_liver_gene_correlations', rep('_healthy', healthy), '.txt'))
