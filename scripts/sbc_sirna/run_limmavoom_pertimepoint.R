...
runDE_timepoint = function(tp, sbc_sym, covcorrect = F, covariate = "") {
	...
	write.table(stripVersion(rownames(y)[keepgenes]), 
				paste0("../../data/expressed_genes_", tp, "_", sbc_sym, ".txt"), 
...
for (tp in c("Baseline", "24h", "4D", "7D")) {
	for (sbc_sym in c("CCDC80", "SOD3")) {
		...
		# 	} else { covcorrect = T }
			covariate = ""
			covcorrect = F
			ext = paste0(tp, "_", sbc_sym, rep(paste0("_covcorrected_", covariate), covcorrect))
			# print(ext)
			result = runDE_timepoint(tp, sbc_sym, covcorrect, covariate)
